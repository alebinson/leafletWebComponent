...
export function extendBubbleLayer(L) {
    L.BubbleLayer = (L.Layer ? L.Layer : L.Class).extend({
        // options: {
        //  property: REQUIRED
        //  legend : true,
        //  max_radius: 40,
        //  scale: <chroma-ts color scale>,
        ...
        onAdd: function (map) {
            ...
            if (this.options.legend) {
                this.showLegend(this._scale, this._max, this._map);
            }
        ...
        createLayer: function () {
            ...
            return new L.geoJson(this._geojson, {
                ...
                pointToLayer: function (feature, latlng) {
                    ...
                    //ALEX
                    style.color = chroma.color(style.fillColor).darker().hex();
        ...
        showLegend: function(scale, max, map){
          ...
          legend.onAdd = function(map) {
            ...
            // For formating was used ---> numeral( max / i ).format('0 a')
              item.innerHTML = '<svg height="' + (max_radius * 2)  +'" width="' + (max_radius * 2 - (max_radius / 2)) + '">' +
                '<circle cx="' + (radius + 1) + '" cy="' + max_radius + '" r="' + radius + '" stroke="' + chroma.color(fill).darker().hex() + '" stroke-width="1" opacity="' + opacity +'" fill="' + chroma.color(fill) +'" />' +
                 '<text font-size="11" text-anchor="middle" x="' + (radius) + '" y="' + (max_radius * 2) + '" fill="#AAA">' +  max / i   + '</text>' +
              '</svg>';
